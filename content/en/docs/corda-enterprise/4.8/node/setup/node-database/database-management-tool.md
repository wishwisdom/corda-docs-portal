...
The database management tool is distributed as a standalone `.jar` file named `tools-database-manager-${corda_version}.jar`. It is intended to be used by Corda Enterprise node administrators who want more control over database changes made in production
environments.
...
bash and zsh. See [Shell extensions for CLI Applications]({{% ref "../../operating/cli-application-shell-extensions.md" %}}) for more info.
