...
* A folder entitled `cordapps` containing any CorDapp `.jar` files you want the node to load
...
### Signing CorDapp `.jar` files

The default behaviour of Cordform is to deploy CorDapp `.jar` files “as built”:



* prior to Corda 4 all CorDapp `.jar` files were unsigned.
* as of Corda 4, CorDapp `.jar` files created by the Gradle *cordapp* plugin are signed by a Corda development certificate by default.


The Cordform `signing` entry can be used to override and customise the signing of CorDapp `.jar` files.
